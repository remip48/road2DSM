...
#'
#' @examples
extract_nc <- function(nc.path,
                       list_variable,
                       nc_files, # dataframe containing file = NC file names, date_start, date_end, type predictors (center, etc, pasted and separated by a ','), and expr if any
                       all_pixel.radius,
                       all_time.period,
                       dates,
                       lonmin = -Inf,
  ...
  order_dim <- function(infos_dimensions, lon, lat, depth, time) {
    out <- c()
  ...
  create_dim <- function(infos_dimensions, lon, lat, depth, time) {
  ...
  nc_filesi <- map_dfr(1:nrow(nc_files), function(i) {
    ...
    dt <- length(as.character(timeSequence(from = nc_files$date_start[i],
                                           ...
                                           by = "day")))
    if (lt != dt) {
      cat("Number of days in", nc_files$file[i], "entered (", dt, ") is different from the NC file (", dt, ").\nFunction continues but stop it if you want to check.\n\n")
    }

    return(nc_files[i, ] %>%
             dplyr::group_by(file, date_start, date_end) %>%
  ...
  list_variable <- nc_filesi %>%
    ...
    ungroup() %>%
    dplyr::mutate(file_set = paste0("file_set_", match(allfile, unique(allfile)))) %>%
    left_join(nc_filesi %>%
                dplyr::select(file, variable) %>%
                distinct(),
              by = "variable",
  ...
  for (f in unique(list_variable$file_set)) {
    ...
    dates <- datesi %>%
      group_by(dates) %>%
      dplyr::mutate(period = if (any(ncinfoi$date_start <= dates & ncinfoi$date_end >= dates)) {
        ncinfoi %>%
          dplyr::filter(date_start <= dates & date_end >= dates) %>%
          pull(period) %>%
          unique()
      } else {
        NA
      ...
      out <- ncinfoi[i, ] %>%
        left_join(map_dfr(v, function(x) {
          map_dfr(x$dim, function(d) {
            return(data.frame(variable = x$name,
                              dim = d$name,
        ...
        # group_by(variable, nc.name) %>%
        # dplyr::summarise(dim = paste(dim, collapse = ", ")) %>%
        ungroup()
    ...
    if (!file.exists(paste0(nc.path, "/", f, ".shp"))) {
      ...
      write_sf(new_grid,
               paste0(nc.path, "/", f, ".shp"),
               append = F)

      cat("Grid from the NC file_set saved under", paste0(nc.path, "/", f, ".shp"), "\n")
    } else {
    ...
    if (!dir.exists(paste0(nc.path, "/", f))) {
      ...
      cat("Extraction will be saved under", paste0(nc.path, "/", f), "\n")
    }

    origin_all <- timeDate(ncinfoi %>%
                             dplyr::slice_min(period) %>%
                             pull(date_start) %>%
                             unique())
    ...
    all_days_period_ref <- data.frame(date = as.character(timeSequence(from = as.character(origin_all),
                                                                       to = as.character(last(dates$dates)),
                                                                       by = "day"))) %>%
      group_by(date) %>%
      dplyr::mutate(period = if (any(ncinfoi$date_start <= date & ncinfoi$date_end >= date)) {
        ncinfoi %>%
          dplyr::filter(date_start <= date & date_end >= date) %>%
          pull(period) %>%
          unique()
      } else {
        NA
    ...
    try({all_run <- foreach(dt = 1:nrow(dates),
                            .packages = c("purrr", "dplyr", "timeDate", "ncdf4", "stringr", "data.table", "collapse"), # matrixStats, rlang
                            .noexport = ls()[!(ls() %in% c("data", "ncinfoi", "nc.path", "expr", "order_dim", "create_dim",
                                                           "origin_all", "all_days_period_ref", "infos_dim", "f", "all_pixel.radius",
                                                           "pixel.radius", "pred.type", "Predictor.name_ref", "all_time.period", "dates"))]) %dopar% {

                                                             file_to_save <- paste0(nc.path, "/", f, "/", dates$dates[dt], "_variables.rds")
                                                             ...
                                                             if (file.exists(file_to_save)) {
                                                               ...
                                                               # origin<-timeDate(paste0(segyear[dt],"-01-01"))                # get Julian day

                                                               origin <- timeDate(ncinfoi %>%
                                                               ...
                                                               SegDay <- length(as.character(timeSequence(from = as.character(origin),
                                                                                                          to = as.character(dates$dates[dt]),
                                                                                                          by = "day")))

                                                               # index of day in the nc file

                                                               all_days_period <- all_days_period_ref %>%
                                                                 dplyr::filter(date <= dates$dates[dt]) %>%
                                                                 dplyr::mutate(id = 1:n()) %>%
                                                                 dplyr::filter(id %in% (n() - max(all_time.period) + 1):n())

                                                               ncfile<-unique(ncinfoi$nc.name[ncinfoi$period==prd])       # extract nc.name as file is listed in nc.path
                                                               datafile <- paste(nc.path,ncfile,sep="/")
                                                               if (!file.exists(datafile)) {
                                                                 return("file did not exist")
                                                               }                          # Open matched .nc file and process data as needed
                                                               nc.data <- nc_open(datafile)

                                                               namedims <- names(nc.data$dim)
                                                               lat <- ncvar_get(nc.data, namedims[str_detect(tolower(namedims), "lat")])
                                                               lon <- ncvar_get(nc.data, namedims[str_detect(tolower(namedims), "lon")])
                                                               if (any(str_detect(tolower(namedims), "depth"))) {
                                                                 depth <- ncvar_get(nc.data, namedims[str_detect(tolower(namedims), "depth")])
                                                               } else {
                                                                 depth = 0
                                                               }
                                                               time <- ncvar_get(nc.data, namedims[str_detect(tolower(namedims), "time")])
                                                               time <- 0:(length(time) - 1)

                                                               #day.index <- which(time==SegDay)                       # changed file.index to day.index; match nc date to SegDay
                                                               day.index <- which((time +1)==SegDay)                 # since 01.01.YY = 0, 02.01.YY =1
                                                               ...
                                                               nrows  <- length(lon)
                                                               ncols  <- length(lat)
                                                               ntimes <- length(time)  # days in month

                                                               time1 <- max(day.index-max(all_time.period)+1,1)                     # days needed to go backwards for SDTime
                                                               ...
                                                               data.var_refi  <-  lapply(Predictor.name, function(pred) {
                                                                 # print(pred)
                                                                 data.var_ref <- ncvar_get(nc.data, pred, start = order_dim(infos_dim %>%
                                                                                                                              dplyr::filter(variable == pred &
                                                                                                                                              nc.name == ncfile),
                                                                                                                            lon = 1, lat = 1, depth = 1, time = time1),
                                                                                           count = order_dim(infos_dim %>%
                                                                                                               dplyr::filter(variable == pred &
                                                                                                                               nc.name == ncfile),
                                                                                                             lon = nrows, lat = ncols, depth = length(depth), time = numtimes),
                                                                                           verbose = FALSE)

                                                                 if (any(all_days_period$period < prd)) {
                                                                   last_period <- unique(all_days_period$period[all_days_period$period < prd])
                                                                   ## working only for 1 period

                                                                   ncfile1 <- ncinfoi$nc.name[ncinfoi$period == last_period]
                                                                   datafile1 <- paste(nc.path, ncfile1,sep="/")

                                                                   nc.data1 <- nc_open(datafile1)                          # Open matched .nc file and process data as needed

                                                                   time11 <- ncvar_get(nc.data1,'time')
                                                                   time11 <- 0:(length(time11) - 1)

                                                                   SegDay1 <- length(as.character(timeSequence(from = ncinfoi$date_start[ncinfoi$period == last_period],
                                                                                                               to = last(all_days_period$date[all_days_period$period == last_period]),
                                                                                                               by = "day")))

                                                                   #day.index <- which(time==SegDay)                       # changed file.index to day.index; match nc date to SegDay
                                                                   day.index1 <- which((time11 +1)==SegDay1)                 # since 01.01.YY = 0, 02.01.YY =1
                                                                   if (length(day.index1)==0) {
                                                                     print(paste('Not OK! Missing dates in nc file for: ',SegDay1,sep="  "))
                                                                   }

                                                                   # ntimes <- length(time1)

                                                                   time11 <- max(day.index1-(max(all_time.period) - numtimes) +1,1)                     # days needed to go backwards for SDTime
                                                                   numtimes11 <- min(max(all_time.period) - numtimes,day.index1)

                                                                   data.var1  <-  ncvar_get(nc.data1, pred, start = order_dim(infos_dim %>%
                                                                                                                                dplyr::filter(variable == pred &
                                                                                                                                                nc.name == ncfile1),
                                                                                                                              lon = 1, lat = 1, depth = 1, time = time11),
                                                                                            count = order_dim(infos_dim %>%
                                                                                                                dplyr::filter(variable == pred &
                                                                                                                                nc.name == ncfile1),
                                                                                                              lon = nrows, lat = ncols, depth = length(depth), time = numtimes11),
                                                                                            verbose = FALSE)
                                                                   ...
                                                                   # rm(nc.data1)
                                                                   nc_close(nc.data1)
                                                                   # target.day <- numtimes + numtimes11
                                                                 }

                                                                 out <- cbind(data.var_ref) %>%
                                                                   as.data.frame() %>%
                                                                   cbind(create_dim(infos_dim %>%
                                                                                      dplyr::filter(variable == pred &
                                                                                                      nc.name == ncfile),
                                                                 ...
                                                                 # ggplot() +
                                                                 #   geom_point(data = data, aes(x = lon_cent, y = lat_cent), color = "red") +
                                                                 #   geom_point(data = out, aes(x = lon, y = lat), size = .1) +
                                                                 #   facet_wrap(~ t)
                                                               ...
                                                               if (length(Predictor.name) > 1) {
                                                                 for (i in 2:length(data.var_refi)) {
                                                                   ...
                                                                   data.var_ref <- data.var_ref %>%
                                                                     left_join(data.var_refi[[i]], by = c("lon", "lat", "t", "d"))
                                                                 }
                                                               }
                                                               ...
                                                               if (any(!(round(data$lat_cent, 6) %in% unique(round(data.var_ref$lat, 6)))) |
                                                                   any(!(round(data$lon_cent, 6) %in% unique(round(data.var_ref$lon, 6))))) {
                                                               ...
                                                               data.var_ref <- data %>%
                                                                 ...
                                                                 left_join(data.var_ref %>%
                                                                             dplyr::mutate(lon_cent = round(lon, 6),
                                                                                           lat_cent = round(lat, 6)),
                                                               ...
                                                               run_mean_SDspace <- any(c("mean", "SDspace") %in% pred.type)

                                                               # summary_exprs <- if (run_mean_SDspace) {
                                                               #   quos(
                                                               #     across(
                                                               #       all_of(Predictor.name),
                                                               #       list(
                                                               #         mean    = ~ mean(.x, na.rm = TRUE),
                                                               #         SDspace = ~ sd(.x, na.rm = TRUE)
                                                               #       ),
                                                               #       .names = "{.col}.{.fn}"
                                                               #     )
                                                               #   )
                                                               ...
                                                               final <- map(all_pixel.radius, function(pixel.radius) {
                                                                 ...
                                                                 res_lat <- mean(sort(lat)[-1] - sort(lat)[-length(lat)], na.rm = T) * (pixel.radius + .5)
                                                                 res_lon <- mean(sort(lon)[-1] - sort(lon)[-length(lon)], na.rm = T) * (pixel.radius + .5)

                                                                 outM <- map_dfr(unique(data$lon_cent), function(l) {
                                                                   # print(match(l, unique(data$lon_cent)))
                                                                   ...
                                                                   left_join(data.var_ref, by = "id_nc", relationship = "many-to-many") %>%
                                                                   dplyr::mutate(dist = abs(lat_cent - lat) + abs(lon_cent - lon)) %>%
                                                                   dplyr::select(id, t, id_nc, dist, all_of(Predictor.name))

                                                                 setDT(outM)

                                                                 # outM <- outM[, c(
                                                                 #   .(id_nc = id_nc[which.min(dist)]),
                                                                 #   setNames(lapply(.SD, mean, na.rm = TRUE), paste0(Predictor.name, ".mean")),
                                                                 #   setNames(lapply(.SD, sd,   na.rm = TRUE), paste0(Predictor.name, ".SDspace"))
                                                                 # ), by = .(id, t), .SDcols = Predictor.name]

                                                                 # outMi <- outM[, c(
                                                                 #   .(id_nc = id_nc[which.min(dist)]),
                                                                 #   unlist(lapply(seq_along(.SD), function(j) {
                                                                 #     x <- .SD[[j]]
                                                                 #     nm <- Predictor.name[j]
                                                                 #     list(
                                                                 #       setNames(mean(x, na.rm = TRUE), paste0(nm, ".mean")),
                                                                 #       setNames(sd(x,   na.rm = TRUE), paste0(nm, ".SDspace"))
                                                                 #     )
                                                                 #   }), recursive = FALSE)
                                                                 # ), by = .(id, t), .SDcols = Predictor.name]
                                                                 ...
                                                                 if (all(c("SDspace", "mean") %in% pred.type)) {
                                                                   outM <- outM[,
                                                                                c( .(id_nc = id_nc[which.min(dist)]),
                                                                                   setNames(fmean(.SD, na.rm = TRUE), paste0(Predictor.name, ".mean")),
                                                                                   setNames(fsd(.SD, na.rm = TRUE), paste0(Predictor.name, ".SDspace")) ),
                                                                                by = .(id, t), .SDcols = Predictor.name]
                                                                 ...
                                                                 #       lon_cent = .x$lon_cent,
                                                                 #       id = .y$id,
                                                                 #       id_nc = paste(data.var_ref_t1$id_nc[abs(.x$lat_cent - data.var_ref_t1$lat) <= res_lat &
                                                                 #                                             abs(.x$lon_cent - data.var_ref_t1$lon) <= res_lon], collapse = ",")
                                                                 #       )
                                                                 #   }) %>%
                                                                 #   bind_rows() %>%
                                                                 #   group_by(id) %>%
                                                                 ...
                                                                 #   dplyr::mutate(dist = abs(lat_cent - lat) + abs(lon_cent - lon)) %>%
                                                                 #   group_by(id, t) %>%
                                                                 #   dplyr::summarise(id_nc = id_nc[which.min(dist)],
                                                                 #                    !!!summary_exprs,
                                                                 #                    .groups = "drop")

                                                                 if (run_mean_SDspace) {
                                                                   colnames(outM)[str_detect(colnames(outM), fixed(".mean")) |
                                                                                    str_detect(colnames(outM), fixed(".SDspace"))] <- paste0(colnames(outM)[str_detect(colnames(outM), fixed(".mean")) |
                                                                                                                                                              str_detect(colnames(outM), fixed(".SDspace"))], "_", pixel.radius, "p")
                                                                 }

                                                                 outM <- outM %>%
                                                                   left_join(data.var_ref %>%
                                                                               dplyr::select(id_nc, t, all_of(Predictor.name)), by = c("t", "id_nc")) %>%
                                                                   dplyr::select(-id_nc)

                                                                 setDT(outM)

                                                                 # summary_exprs1 <- if (run_mean_SDspace) {
                                                                 #   quos(
                                                                 #     across(all_of(colnames(outM)[str_detect(colnames(outM), fixed(".mean")) |
                                                                 #                                    str_detect(colnames(outM), fixed(".SDspace"))]),
                                                                 ...
                                                                 # }
                                                                 #
                                                                 # summary_exprs2 <- if (pixel.radius == all_pixel.radius[1]) {
                                                                 #   quos(
                                                                 #     across(all_of(Predictor.name),
                                                                 #            list(center = ~ mean(.x, na.rm = T),
                                                                 #                 SDtime = ~ sd(.x, na.rm = T)),
                                                                 #            .names = "{.col}.{.fn}")
                                                                 #   )
                                                                 # }
                                                                 ...
                                                                 cols1 <- colnames(outM)[
                                                                   stringr::str_detect(colnames(outM), fixed(".mean")) |
                                                                     stringr::str_detect(colnames(outM), fixed(".SDspace"))
                                                                 ]
                                                                 ...
                                                                 outfinal <- map(all_time.period, function(time.period) {
                                                                   ...
                                                                   # outf <- outM %>%
                                                                   #   dplyr::filter(t %in% (max(all_time.period) - time.period + 1):numtimes) #%>%
                                                                   # group_by(id) %>%
                                                                   # dplyr::summarise(!!!summary_exprs1,
                                                                   #                  !!!summary_exprs2) %>%
                                                                   # ungroup()

                                                                   if (time.period == 1) {
                                                                     outf <- outM[t %in% (max(all_time.period) - time.period + 1):numtimes, -c("t")]

                                                                     colnames(outf)[colnames(outf) %in% cols1] <- paste0(cols1, ".mean")
                                                                     colnames(outf)[colnames(outf) %in% cols2] <- paste0(cols2, ".center")
                                                                     ...
                                                                     if (any(c("SDspace", "mean") %in% pred.type) &
                                                                       ...
                                                                       outf <- outM[
                                                                         ...
                                                                         c(
                                                                           setNames(lapply(.SD[, ..cols1], mean, na.rm = TRUE), paste0(cols1, ".mean")),
                                                                           setNames(lapply(.SD[, ..cols1], sd,   na.rm = TRUE), paste0(cols1, ".SDtime")),
                                                                           setNames(lapply(.SD[, ..cols2], mean, na.rm = TRUE), paste0(cols2, ".center")),
                                                                           setNames(lapply(.SD[, ..cols2], sd,   na.rm = TRUE), paste0(cols2, ".SDtime"))
                                                                         ),
                                                                         by = id
                                                                       ]
                                                                     } else if (any(c("SDspace", "mean") %in% pred.type)) {
                                                                       outf <- outM[
                                                                         t %in% (max(all_time.period) - time.period + 1):numtimes,
                                                                         c(
                                                                           setNames(lapply(.SD[, ..cols1], mean, na.rm = TRUE), paste0(cols1, ".mean")),
                                                                     ...
                                                                     } else if (any(c("center", "SDtime") %in% pred.type)) {
                                                                       outf <- outM[
                                                                         t %in% (max(all_time.period) - time.period + 1):numtimes,
                                                                         c(
                                                                           setNames(lapply(.SD[, ..cols2], mean, na.rm = TRUE), paste0(cols2, ".center")),
                                                                   ...
                                                                   colnames(outf)[2:ncol(outf)] <- paste0(colnames(outf)[2:ncol(outf)], "_", time.period, "d")
                                                               ...
                                                               # ggplot() +
                                                               #   geom_point(data = out_final, aes(x = lon_cent, y = lat_cent, color = thetao.SDspace_2p.SDtime_8d)) +
                                                               #   scale_color_viridis_c()

                                                               saveRDS(out %>%
                                                                         mutate(date = dates$dates[dt]),
                                                                       file_to_save)
